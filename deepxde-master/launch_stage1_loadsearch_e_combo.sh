...
SMOKE_GROUP=Stage1-LoadSearch-Smoke2k
...
launch_one() {
  ...
  mkdir -p "$smoke_dir" "$formal_dir" "$ROOT/exp/$SMOKE_GROUP/$case_name" "$ROOT/exp/$FORMAL_GROUP/$case_name"

  smoke_cmd=$(build_cmd python train_material_field.py "${COMMON_ARGS[@]}" --case "$case_name" --observation_split_tag "$obs_tag" --load_modes "$load_modes" --load_scales "$load_scales" --experiment_group "$SMOKE_GROUP" --run_name "${run_key}_smoke2k" "${SMOKE_ARGS[@]}")
...
run_stage '${smoke_dir}/screen.log' ${smoke_cmd}
run_stage '${formal_dir}/screen.log' ${formal_cmd}
EOF
  chmod +x "$wrapper"
