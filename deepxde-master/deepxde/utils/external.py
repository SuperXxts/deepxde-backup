...
def saveplot(
    ...
    if isplot:
        ...
        loss_fig_fname = os.path.join(output_dir, "loss_history.png") if issave else None
        plot_loss_history(loss_history, fname=loss_fig_fname)
        
        # Save best state plot(s)
        if train_state.X_test is not None and train_state.X_test.shape[1] <= 2:
            # For 1D or 2D problems, save the plot
            if train_state.X_test.shape[1] == 1:
                best_fig_fname = os.path.join(output_dir, "best_state.png") if issave else None
            else:  # 2D
                # Get y_dim from train_state
                _, _, best_y, _ = _pack_data(train_state)
                y_dim = best_y.shape[1] if best_y is not None else 1
                best_fig_fname = [os.path.join(output_dir, f"best_state_{i+1}.png") for i in range(y_dim)] if issave else None
            plot_best_state(train_state, fname=best_fig_fname)
...
def plot_best_state(train_state, fname=None):
    ...
    Args:
        ...
        fname (string): If `fname` is a string (e.g., 'best_state.png'), then save the
            figure to the file of the file name `fname`. If `fname` is a list of strings,
            save each figure to the corresponding file name.
    """
