...
launch_one 10.6.234.22 1 rs0_stagephys_only_60k ""
launch_one 10.6.234.23 1 rs1_split_freeze_60k "${RESP_FREEZE}"
launch_one 10.6.234.24 1 rs2_split_bin_60k "${RESP_FREEZE} ${RESP_BIN}"
launch_one 10.6.234.25 1 rs3_split_bin_contrast_60k "${RESP_FREEZE} ${RESP_BIN} ${RESP_CONTRAST}"
launch_one 10.6.234.27 1 rs4_split_bin_contrast_lock4_60k "${RESP_FREEZE} ${RESP_BIN} ${RESP_CONTRAST} ${RESP_LOCK4}"
launch_one 10.6.234.28 1 rs5_split_bin_contrast_lock8_60k "${RESP_FREEZE} ${RESP_BIN} ${RESP_CONTRAST} ${RESP_LOCK8}"
launch_one 10.6.234.29 1 rs6_split_bin_contrast_lock8_longgeom_60k "${RESP_FREEZE} ${RESP_BIN} ${RESP_CONTRAST} ${RESP_LOCK8} ${RESP_LONG}"
launch_one 10.6.234.30 1 rs7_split_bin_contrast_lock8_sharp_60k "${RESP_FREEZE} ${RESP_BIN} ${RESP_CONTRAST} ${RESP_LOCK8} ${RESP_SHARP}"
