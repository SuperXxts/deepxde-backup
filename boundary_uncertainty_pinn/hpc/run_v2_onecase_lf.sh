...
case "$CASE" in
  full_boundary_oracle|full_boundary_oracle_reaction|correct_top_amp_only|correct_top_amp_only_reaction|wrong_fixed_A|wrong_fixed_A_reaction|learnable_A|learnable_A_anchor|learnable_A_reaction|learnable_A_anchor_reaction|oracle_A|full_boundary_aware)
    ;;
  *)
...
case "$MATERIAL_MODE" in
  single|dual)
    ;;
  *)
    echo "ERROR: unsupported MATERIAL_MODE=$MATERIAL_MODE" >&2
    exit 2
    ;;
esac
