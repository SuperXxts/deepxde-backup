...
正式 FEM 实验默认采用三工况共享材料场，而不是把单工况和三工况作为论文主线对比。原因是：在空间变双参数材料场 `K(x,y), mu(x,y)` 反演中，单一加载工况通常不足以稳定提供体积变形和剪切变形信息；多工况共享同一材料场更符合岩土/固体反演中的可识别性设计习惯。

因此，三工况在本文中只作为严谨实验设计的基础条件，不作为创新点。本文真正比较的是：在三工况信息已经相对充分的前提下，边界条件错误或不完整时，标准 PFNN 及其扩展是否仍会把边界误差吸收到材料场，以及低维边界分支、反力约束、边界锚点和解耦训练是否能改善这种污染。

正式 FEM 主线编号如下：

```text
B0: 三工况 + 真实边界，作为理想边界上限参考。
B1: 三工况 + 错误固定边界，验证边界错误是否污染 K/mu 反演。
B2: 三工况 + 可学习边界幅值，验证低维边界校准是否有效。
B3: 三工况 + 可学习边界幅值 + 总反力，验证反力对绝对刚度尺度的约束作用。
B4: 三工况 + 可学习边界幅值 + 总反力 + 边界锚点，验证少量边界位移信息的增量作用。
B5: 三工况 + 边界分支 + 总反力 + 边界锚点，验证低维边界分支是否优于单一幅值参数。
B6: 三工况 + 边界分支 + 总反力 + 边界锚点 + 解耦训练，验证完整方法。
```

B2-B4 中的“可学习边界幅值”使用每个加载工况的外部可训练标量参数。B5-B6 不再额外叠加外部幅值参数，而是由边界分支的常数模态承担幅值校准，并允许低维形状模态表达加载边界的形状误差。这样可以避免同一个边界幅值被两个参数通道重复学习。

单工况实验只保留为内部诊断或补充说明，不进入正式主表，不作为本文创新故事。
